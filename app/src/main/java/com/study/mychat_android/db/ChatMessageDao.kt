...
interface ChatMessageDao{
    ...
    @Query("SELECT * FROM chat_message WHERE ower_id = :owerId AND target_id = :targetId")
    fun getChatMessageList(owerId:Int,targetId :Int):List<ChatMessage>

    @Query("SELECT * FROM chat_message WHERE ower_id = :owerId AND target_id = :targetId ORDER BY creat_time")
    fun getChatMessageList2(owerId:Int,targetId :Int):LiveData<List<ChatMessage>>

    @Query("SELECT * FROM chat_message WHERE ower_id = :targetId AND target_id = :owerId ORDER BY creat_time")
    fun getChatMessageList3(owerId:Int,targetId :Int):LiveData<List<ChatMessage>>
